class Users {
  ...
  async ensureAdmin(email, password) {
    ...
    if (!user) {
      ...
      if (user._id)
        await this.setUserPassword(user._id, password);
    }
  }
  ...
  async findOrCreateService(email, service, id, profile, accessToken, refreshToken) {
    ...
    if (service)
      query.$or.push({ $and: [ { "services.service": service }, { "services.id": id }]});
    ...
    if (user) {
      ...
      user.services = user.services.filter(s => !(s.service === service && s.id === id));
